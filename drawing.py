...
class Drawing:
    def __init__(self, sc, sc_map, player, clock):
        ...
        self.textures = {1: pygame.image.load('textures/wall8.png').convert(),
                         2: pygame.image.load('textures/wall4.png').convert(),
                         3: pygame.image.load('textures/wall5.png').convert(),
                         4: pygame.image.load('textures/wall6.png').convert(),
                         'S': pygame.image.load('textures/sky1.png').convert()
                         }
        # menu
        self.menu_trigger = True
        self.pause_trigger = True
        self.menu_picture = pygame.image.load('textures/popularity_by_lordnetsua-d4qd6qx.jpg').convert()
        # weapon parameters
    ...
    def pause(self):
        ...
        start = button_font.render('NO', 1, pygame.Color('lightgray'))
        button_start = pygame.Rect(0, 0, 500, 150)
        button_start.center = HALF_WIDTH, HALF_HEIGHT
        exit = button_font.render('YES', 1, pygame.Color('lightgray'))
        button_exit = pygame.Rect(0, 0, 500, 150)
        button_exit.center = HALF_WIDTH, HALF_HEIGHT + 200
        if keys[pygame.K_ESCAPE]:
            while self.menu_trigger:
                ...
                self.sc.blit(start, (button_start.centerx - 130, button_start.centery - 35))

                pygame.draw.rect(self.sc, BLACK, button_exit, border_radius=25, width=10)
                self.sc.blit(exit, (button_exit.centerx - 85, button_exit.centery - 35))

                color = randrange(150, 255, 3)
                label = label_font.render('Exit?', 1, (color, color, color))
    ...
    def menu(self):
        ...
        while self.menu_trigger:
            ...
            color = randrange(150, 255, 3)
            label = label_font.render('Shoot it', 1, (color, color, color))
            self.sc.blit(label, (15, -30))
